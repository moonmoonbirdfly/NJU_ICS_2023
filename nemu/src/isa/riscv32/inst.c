...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000000 ????? ????? 000 ????? 01100 11", addi   , I, R(rd) = src1 + imm); // 匹配 addi 指令，执行 R(rd) = src1 + imm 的操作
  ...
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu , R, R(rd) = (long long)(src1 * src2 )>> 32); // 匹配 mulhsu 指令，执行 R(rd) = (sword_t)src1 * src2 >> 32 的操作 
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, R(rd) = (long long)(src1 * src2) >> 32); // 匹配 mulhu 指令，执行 R(rd) = src1 * src2 >> 32 的操作 
